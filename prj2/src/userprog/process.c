...
void build_esp(char *filename, void **esp){
	char *cmd, *next;
	char *argv[50];
	char fn_copy[256];
	int argc = 0 , i, len, total_len = 0;
	strlcpy(fn_copy, filename, PGSIZE);
	cmd = strtok_r(fn_copy, " ", &next);
	argv[argc++] = cmd;

	while(1){
		cmd = strtok_r(NULL, " ", &next);
		if(cmd == NULL)
			break;
		argv[argc++] = cmd;
	}
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  bool flag = false;
  //int temp = 0;
  //elem = list_begin(&(curr->child_list));
  for(elem = list_begin(&(curr->child_list)); elem != list_end(&(curr->child_list)); elem = list_next(elem)){
	...
	else if(child_tid == thr->tid){
		//temp = thr->tid;
		sema_down(&(thr->c_lock));
